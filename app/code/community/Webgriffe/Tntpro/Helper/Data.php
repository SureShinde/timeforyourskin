...
class Webgriffe_Tntpro_Helper_Data extends Mage_Core_Helper_Data
{
    ...
    protected function _generateXml($obj, $magazzino_default, $magazzino, $params, $b_domestic)
    {
        ...
        if (in_array($params['operationaloption'], array(1,3,5))) { # nel caso di Fermo Deposito, Tnt point o Lokerbox mettiamo l'indirizzo di billing al posto di quello di shipping
            $_addresses["D"] = $this->_createAddressWrapper($_order->getBillingAddress());
        }
        ...
        if ($params['length']) {
            $dimensions->addChild('length', str_pad(intval($params['length'] * 1000), 6, '0', STR_PAD_LEFT));
        }
        if ($params['height']) {
            $dimensions->addChild('height', str_pad(intval($params['height'] * 1000), 6, '0', STR_PAD_LEFT));
        }
        if ($params['width']) {
            $dimensions->addChild('width', str_pad(intval($params['width'] * 1000), 6, '0', STR_PAD_LEFT));
        }
...
//            }
    ...
    public function sendMail( $csv )
    {
        ...
        $mail->setSenderName (Mage::getStoreConfig('shipping/wgtntpro/mailmittente'));
        $mail->setSenderEmail (Mage::getStoreConfig('shipping/wgtntpro/mailmittente'));
        $mail->setReplyTo (Mage::getStoreConfig('shipping/wgtntpro/mailmittente'));

        $mail->setTemplateSubject(Mage::getStoreConfig('shipping/wgtntpro/mailsubject'));
        $mail->setTemplateText('');
        ...
        try {
            return $mail->send(Mage::getStoreConfig('shipping/wgtntpro/maildestinatario'));
        } catch (Zend_Mail_Transport_Exception $e) {
            $this->log('Error while sending mail: '.$e->getMessage(), Zend_Log::CRIT);
            return false;
        }
    }
    ...
    {
        Mage::log($message, $level, self::LOGFILE, Mage::getStoreConfig('shipping/wgtntpro/debug'));
    }
}