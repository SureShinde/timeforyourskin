...
//            return $this;
    ...
    public function orderPlaceBefore($observer) {
        ...
        if ($codes = $session->getGiftCodes()) {
            ...
            foreach ($baseAmountUsed as $code => $amount) {
                ...
                if (!$model || $model->getStatus() != Magestore_Giftvoucher_Model_Status::STATUS_ACTIVE || $model->getBaseBalance() < $amount) {
        ...
        if ($codes = $order->getGiftCodes()) {
            ...
            foreach ($codesArray as $code) {
                ...
                $history = Mage::getModel('giftvoucher/history')->setData(array(
                            ...
                            'comments' => Mage::helper('giftvoucher')->__('Spend on order %s', $order->getIncrementId()),
            ...
            if (Mage::app()->getStore()->roundPrice($order->getGrandTotal()) == 0 && ($order->getPayment()->getMethod() == 'giftvoucher' || $order->getPayment()->getMethod() == 'free') && Mage::getStoreConfigFlag('payment/giftvoucher/invoice') && $order->canInvoice()) {
                try {
                    $itemQtys = array();
    ...
    public function paypalPrepareItems($observer) {
        ...
        if (version_compare(Mage::getVersion(), '1.4.2', '>=')) {
            ...
            if ($paypalCart) {
                ...
                $giftCardDiscount = $salesEntity->getBaseGiftVoucherDiscount();
                if (!$giftCardDiscount)
                ...
                $giftCardCreditAmount = $salesEntity->getBaseUseGiftCreditAmount();
    ...
    public function orderLoadAfter($observer) {
        ...
        if ((abs($order->getGiftVoucherDiscount()) < 0.0001 && abs($order->getUseGiftCreditAmount()) < 0.0001) || Mage::app()->getStore()->roundPrice($order->getGrandTotal()) > 0 || $order->getState() === Mage_Sales_Model_Order::STATE_CLOSED || $order->isCanceled() || $order->canUnhold()) {
            return $this;
        }
        ...
        foreach ($invoice->getAllItems() as $itemCredit) {
            ...
            if (isset($item) && $item != null) {
                if ($item->getProductType() != 'giftvoucher')
                    continue;
                if (!Mage::helper('giftvoucher')->getGeneralConfig('autochange', $order->getStoreId()))
                    return $this;
...
//                $buyRequest['customer_name'] = $data['customer_name'];
//                $buyRequest['email_sender'] = $data['email_sender'];
//                $buyRequest['giftcard_template_id'] = $data['giftcard_template_id'] ? $data['giftcard_template_id'] : Mage::getModel('giftvoucher/gifttemplate')->getCollection()->getFirstItem()->getId();
//                $buyRequest['recipient_name'] = $data['recipient_name'];
//                $buyRequest['recipient_email'] = $data['recipient_email'];
//                $buyRequest['message'] = $data['message'];
//                $buyRequest['day_to_send'] = $data['day_to_send'];
//                $buyRequest['giftcard_template_image'] = $data['gift-image'];
//                $buyRequest['notify_success'] = $data['notify_success'];
//                $buyRequest['recipient_ship'] = $data['recipient_ship'];
//            }
            $giftVouchers = Mage::getModel('giftvoucher/giftvoucher')->getCollection()->addItemFilter($item->getId());
//            if ($order->getStatus() == Mage_Sales_Model_Order::STATE_COMPLETE && Mage::helper('giftvoucher')->getGeneralConfig('autochange', $order->getStoreId())) {
//                foreach ($giftVouchers as $giftVoucher) {
//                    if ($giftVoucher->getStatus() == Magestore_Giftvoucher_Model_Status::STATUS_PENDING) {
//                        $giftVoucher->addData(array(
//                            'status' => Magestore_Giftvoucher_Model_Status::STATUS_ACTIVE,
...
//                    $amount = $amount * 100 / Mage::helper('tax')->getPrice($product, 100, $includeTax);
                ...
                if (isset($buyRequest['giftcard_template_image']) && $buyRequest['giftcard_template_image']) {
                    if (isset($buyRequest['giftcard_use_custom_image']) && $buyRequest['giftcard_use_custom_image']) {
                        ...
                        if (file_exists($dir)) {
                            ...
                            $imageObj->save(Mage::getBaseDir() . str_replace("/", DS, strstr($image_path, '/media')) . $buyRequest['giftcard_template_image']);
                            Mage::helper('giftvoucher')->customResizeImage($image_path, $buyRequest['giftcard_template_image'], 'images');
                            $giftVoucher->setGiftcardCustomImage(true);
                            unlink($dir);
                        } else {
                            $buyRequest['giftcard_template_image'] = 'default.png';
                        }
                    }
                ...
                try {
                    ...
                    $giftVoucher->save();
//                    if (Mage::helper('giftvoucher')->getEmailConfig('enable', $order->getStoreId()) && $order->getStatus() == Mage_Sales_Model_Order::STATE_COMPLETE)
//                        $giftVoucher->sendEmail();
                    if ($order->getCustomerId()) {
                        Mage::getModel('giftvoucher/customervoucher')
                                ->setCustomerId($order->getCustomerId())
    ...
    public function giftcardPaymentMethod($observer) {
        ...
        /* Vick - Fixed Duplicate Coupon Form */
