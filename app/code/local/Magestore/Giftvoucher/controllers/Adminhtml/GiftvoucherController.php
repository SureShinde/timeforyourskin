        ...
        if (isset($_FILES['filecsv'])) {
            try {
                ...
                foreach ($giftVoucherImport as $giftVoucherData) {
                    ...
                    if (!$giftVoucherData['giftcard_template_id']){
                        $giftVoucherData['giftcard_template_id']=Mage::getModel('giftvoucher/gifttemplate')->getCollection()->getFirstItem()->getId();
                    }
                    try {
...
//                if ($order_status == 'pending' && $data['status'] == Magestore_Giftvoucher_Model_Status::STATUS_DISABLED) {
//                    if ($order->canCancel()) {
//                        $order->cancel();
...
//            }
            if (!$data['giftcard_template_id']) {
                $data['giftcard_template_id']=Mage::getModel('giftvoucher/gifttemplate')->getCollection()->getFirstItem()->getId();
            }
            $model->setData($data)
                    ->setIncludeHistory(true)
