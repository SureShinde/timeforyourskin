...
class Peerforest_Megamenu_Block_Topmenu extends Peerforest_Megamenu_Block_Html_Topmenu
{
    ...
    public function getSideHtml($mobile = false)
    {
        $this->_isHomeActive = true;
            
        
        return $this->getSideMegaMenuHtml($mobile);        
    }
    ...
    public function getSideMegaMenuHtml($mobile)
    {
        Mage::dispatchEvent('page_block_html_topmenu_gethtml_before', array(
            'menu' => $this->_menu,
            'block' => $this
    ...
    protected function _getSideMegaMenuHtml(Varien_Data_Tree_Node $menuTree, $levelItem,$columnCount = 0)
    {
       $html = '';
        ...
        foreach ($children as $child) {
            ...
            $parentClass = '';
            if(($childLevel == 0 || $childLevel == 1) && $child->hasChildren()){
                $parentClass = ' parent';
            }
            ...
            if($category->getData('verticalmenu_width')){
                $width = $category->getVerticalmenuWidth().'px';
            }
            ...
            if($childLevel == 0 &&  $this->_verticalMenu == false ){
                ...
                if($addBlock){
                    $html .= '<div id="popup'.$childLevel.$levelItem.'" class="mega-block" style="width:'.$width.'">';
            ...
            if ($child->hasChildren()) {                
                ...
                $html .= $this->_getSideMegaMenuHtml($child, 0, $columnCount);
                $html .= '</ul>';
                if(!$addBlock && $childLevel == 0){
                    $html .= '</div>';
                }
    ...
    protected function _getBlockHtml($content, $key, $width = false)
    {
        ...
        //    $addGutter = ' no-'.$key.'-gutter';
        //}
        if($content[$key]){
            $html .= '<div class="custom-block-'.$key. $addGutter.' grid_'.$setWidth.'">';
            $html .= $content[$key];
            $html .= '</div>';
        }
        return $html;
