...
class AW_Followupemail_Model_Filter extends Mage_Core_Model_Email_Template_Filter
{
    ...
    */
    ...
    public function filter($value)
    {
        ...
        $this->_modifiers['goto'] = array($this, 'modifierGotoParam');
    ...
    public function modifierGotoParam($value)
    {
        $params = func_get_args();
        if (count($params) == 2) {
            $urlModel = Mage::getModel('core/url')->parseUrl($value);
            $path = $urlModel->getPath();
            if ($path[strlen($path) - 1] !== '/') {
                $path .= '/';
            }
            $urlModel->setPath($path . 'goto/' . $params[1] . '/');
            $port = $urlModel->getPort() ? ':' . $urlModel->getPort() : '';
            $url = $urlModel->getScheme() . '://' . $urlModel->getHost() . $port . $urlModel->getPath();
            $query = $urlModel->getQuery();
            if ($query) {
                $url .= '?' . $query;
            }
            $fragment = $urlModel->getFragment();
            if ($fragment) {
                $url .= '#' . $fragment;
            }
            $value = $urlModel->escape($url);
        }
        return $value;
    }
